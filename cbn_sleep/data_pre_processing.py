...
xl = pd.ExcelFile('./cbn_data.xlsx')
df = xl.parse('PII Cohort Data')

anxiety_scores = {
    'columns': [
        "PROMIS_Anx_Fearful", "PROMIS_Anx_AA", "PROMIS_Anx_WO", "PROMIS_Anx_Uneasy",
    ],
    'code': {
        "Always": 5,
        "Often": 4,
...
sleep_scores = {
    'columns_1': ['PROMIS_SD_DiffFalling', 'PROMIS_SD_Problem'],
    'code_1': {
        "Very much": 5,
        "Quite a bit": 4,
        "Somewhat": 3,
    ...
    'columns_2': ['PROMIS_SD_Refreshing'],
    'code_2': {
        "Very much": 1,
        "Quite a bit": 2,
        "Somewhat": 3,
...
df_wellbeing_scores = df[wellbeing_scores['columns']].replace(wellbeing_scores['code']).sum(axis='columns', skipna=False)
