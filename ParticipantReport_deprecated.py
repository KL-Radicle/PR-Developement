...
def get_columns_by_condition(df, condition, numeric_weeks=True):
    ...
    columns = [(condition_flag, s) for s in ['week 0'] + [x.lower() for x in get_weeks(df, numeric_weeks=numeric_weeks)]]
    return columns
    ...
    for i, row in tqdm(df.iterrows(), total=len(df), desc='Participant Report Generation Progress'):
        try:
            ...
            for condition in CONFIG['STUDY_CONDITIONS']:
                ...
                if eligible:
                    patient, cohort_mean, cohort_minmax, flag = get_condition_data_for_patient(df, i, condition, numeric_weeks=False)
