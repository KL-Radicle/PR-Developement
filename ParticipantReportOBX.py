...
from graphing_lib import number_line_plot
...
def generate_section_text_by_condition(condition, hash, text_scores_input_1, text_scores_input_2,
    ...
    baseline_text = [
        ...
        '',
        'Your Baseline Score vs All participants in Control',
        (CONFIG['S3_prefix']) + ('%s/%s_baseline.png' % (hash, condition)),
        text_baseline,
        'Your Score Change vs All participants in Control',
...
def get_columns_by_condition(df, condition, numeric_weeks=True):
    ...
    columns = [(condition_flag, s) for s in ['Week %s' % y for y in [0] + [x for x in range(1, 1 + len(
        get_weeks(df, numeric_weeks=numeric_weeks)))]]]
    return columns
...
def generate_distribution_graph(df, cohort_mean, cohort_minmax, condition, patient, brand, pid, save_path):
    """
    Shows participant in the cohort baseline distribution.
    ...
    patient_score = patient_baseline[0]
    if pd.notna(patient_score):
        patient_score = int(float(patient_score))
        highlight = False
    else:
        patient_score = -1
        highlight = True
    ...
    savepath = save_path / (condition + '_' + 'baseline.png')
    number_line_plot(*cohort_minmax, patient_score, line_pos, savepath, missing_data=False)
...
def generate_score_change_graph(df, flag, cohort_mean, cohort_minmax, condition, patient, brand, save_path):
    ...
    if flag:  # Completed Study. Make line plot
        ...
        plot_line(x_label_list, stacked_y, ylabel=CONFIG['STUDY_CONDITION_TO_STR'][condition] + ' Score', title='',
                  legend=['You', 'All Participants\n in Control'], yrange=y_range)
...
def gen_text(df, patient, cohort, condition):
    ...
    patient_val = pd.to_numeric(patient.squeeze()).ffill()  # DF to list forward filled
    ...
    if patient_val[0] > cohort_val[0]:
        ...
        text_baseline_input = 'missing, and may be more or less than '

    pv2 = patient_val.dropna().tolist()
    # ic(patient_val, pv2)
    patient_val = pv2
...
def generate_graph_and_text(df, patient, cohort_mean, cohort_minmax, condition, completed_study: bool, brand, pid,
    ...
    # generate graph
    condition_minmax = CONFIG['condition_minmax'][condition]
    # generate_distribution_graph(df, cohort_mean, condition_minmax, condition, patient, brand, pid, save_path)
    # TODO
...
def generate_map(df, brandNo, save_path):
    ...
    # get states based off their zip codes
    # ic(df[df[CONFIG['COL_ZIPCODE']].isna()])
    mailing_zips = list(df[CONFIG['COL_ZIPCODE']].dropna())
    # ic(mailing_zips)
    mailing_states = []
...
def generate_participant_report_obx(df, CONFIG, contacts):
    ...
    email_list, errors = [], []
    complete_counts = list(df_completed_weekly[[('Finished', 'Finished', 'OBX_Baseline', 'choice', 7),
